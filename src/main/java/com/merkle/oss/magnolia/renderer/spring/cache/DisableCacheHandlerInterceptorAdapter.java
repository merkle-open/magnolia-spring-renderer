	...
	public void afterCompletion(final HttpServletRequest request, final HttpServletResponse response, final Object handler, final Exception ex) {
		if (handler instanceof HandlerMethod) {
			getAnnotation((HandlerMethod) handler).filter(disableCache -> evaluate(disableCache, request)).ifPresent(disableCache -> {
				MgnlContext.getWebContext().getResponse().setHeader("Cache-Control", "no-cache, no-store, must-revalidate, max-age=" + disableCache.ttlInSeconds());
				MgnlContext.getWebContext().getResponse().setHeader("Pragma", "no-cache");
			});
		}
	}
